...
$pdo=pdo(); $n=$pdo->prepare("SELECT * FROM notifications WHERE id=? AND user_id=?"); $n->execute([$id,$uid]); $row=$n->fetch();
if(!$row) json_response(404,['error'=>'No existe notificación']);
