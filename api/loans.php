...
if (!is_array($books) || count($books) === 0) json_response(400, ['error' => 'Debe seleccionar al menos un libro']);
if (count($books) > 3) json_response(422, ['error' => 'Limite 3 libros']);

$pdo = pdo();
foreach ($books as $bid) {
  $b = $pdo->prepare("SELECT * FROM books WHERE id=?");
  $b->execute([$bid]);
  $bk = $b->fetch();
...
audit('loans.create', ['count' => count($books)]);
notify($uid, 'Prestamo confirmado', 'Su solicitud de prestamo fue procesada.');
