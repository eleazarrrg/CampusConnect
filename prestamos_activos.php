...
async function load(){
  ...
  list.forEach(l=>{ const div=document.createElement('div'); div.className='card'; div.innerHTML=`<strong>${l.title}</strong> - estado: ${l.status} - id:${l.id} <button class='btn' data-id='${l.id}' data-status='${l.status}'>Editar</button>`; c.appendChild(div); });
  [...c.querySelectorAll('button[data-id]')].forEach(b=> b.onclick = ()=> selectLoan(b.getAttribute('data-id'), b.getAttribute('data-status')) );
}
function selectLoan(id,status){
  currentId=id; currentStatus=status; statusSel.value=status||'proceso';
  document.getElementById('delete').style.display = (status==='proceso') ? 'inline-block' : 'none';
}
save.onclick=async()=>{
